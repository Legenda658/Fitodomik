...
class DataSender:
    ...
    def send_data(self, temp, humidity, soil, light, co2, pressure, 
        ...
        try:
            ...
            print(f"Отправляем данные на сервер: {data}")
            response = requests.post(
                self.server_url, 
                data=data, 
            ...
            if response.status_code == 200:
                try:
                    ...
                    if resp_data.get('success'):
                        ...
                        print(f"Данные успешно отправлены, ID: {next_id}")
                        return True
                    else:
                        print(f"Сервер вернул ошибку: {resp_data.get('message', 'Неизвестная ошибка')}")
                        return False
                except Exception as e:
                    print(f"Ошибка декодирования ответа сервера: {str(e)}")
            ...
            print(f"Ошибка при отправке данных на сервер: {str(e)}")
            return False
class ThresholdManager:
    ...
    def get_thresholds(self):
        if not self.token:
            return None
        try:
            response = requests.get(self.thresholds_url, headers=self.headers)
            if response.status_code == 200:
                try:
                    data = response.json()
                ...
                print(f"Ошибка получения порогов: {response.status_code}")
                return None
        except Exception as e:
            print(f"Ошибка при получении порогов: {str(e)}")
            return None
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data.get('success'):
                        ...
                        print(f"Ошибка получения расписания: {data.get('message', 'Неизвестная ошибка')}")
                        return None
                except Exception as e:
                    print(f"Ошибка декодирования ответа сервера при получении расписания: {str(e)}")
                    return None
            elif response.status_code == 401:
                return None
            else:
...
class FitoDomikApp:
    ...
    def auto_mode_loop(self):
        print("Запущен цикл автоматического режима")
        ...
        if self.thresholds_mode.get() == "auto":
            ...
            if not schedule_data:
                print("Не удалось получить расписание с сервера")
        else:
        ...
        while self.auto_mode_running:
            try:
                ...
                try:
                    ...
                    print(f"Ошибка обработки датчиков: {str(e)}")
            except Exception as e:
                print(f"Ошибка в цикле автоматического режима: {str(e)}")
            self.root.after(0, self.update_control_display)
            time_module.sleep(10)
        print("Цикл автоматического режима остановлен")
                ...
                if has_valid_data:
                    try:
                        ...
                        if success:
                            ...
                            print(f"Данные успешно отправлены на сервер в {current_time.strftime('%H:%M:%S')}")
                        else:
                            self.last_send_time = current_time - timedelta(seconds=(self.data_send_interval - 60))
                            print(f"Ошибка отправки данных на сервер. Повторная попытка через 60 секунд.")
                    except Exception as e:
                        print(f"Исключение при отправке данных: {str(e)}")
                        self.last_send_time = current_time - timedelta(seconds=(self.data_send_interval - 60))
        except Exception as e:
            print(f"Ошибка в update_monitoring_display: {str(e)}")
    ...
    def _fetch_weather_data(self):
        try:
            response = requests.get(
                f'https://api.weatherapi.com/v1/current.json?key={self.weather_api_key}&q=Moscow&lang=ru'
            )
            data = response.json()
            ...
            icon_response = requests.get(f"https:{icon_url}")
    ...
    def init_settings_tab(self):
        ...
        polling_label = tk.Label(polling_frame, text="Опрос:", font=("Arial", 12))
